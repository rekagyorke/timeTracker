(function() {
    ...
    function time($resource) {
        ...
        var Time = $resource('api/tasks/:id', {}, {
            update: {
                method: 'PUT'
            }
        });
        ...
        function getTime(project_id, user_id, job) {
            if (job==2) {
                return Time.query({project_id: project_id}).$promise.then(function(results) {
                ...
                return Time.query({project_id: project_id, user_id: user_id}).$promise.then(function (results) {
                    angular.forEach(results, function (result) {
